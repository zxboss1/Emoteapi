...
class CLIENT:
            ...
            try:
                if not command_queue.empty():
                    ...
                    if command_type == 'join_and_emote':
                        ...
                        # Wait for team join - REDUCED from 2 seconds to 1 second
                        ...
                        for uid in uids:
                            if uid:
                                ...
                                # REDUCED from 0.3 seconds to 0.15 seconds between emotes
                                await asyncio.sleep(0.15)
                        
                        # Leave squad after emotes - REDUCED from 1 second to 0.5 seconds
    ...
    async def MaiiiinE(self):
        Uid, Pw = '4232977194','JOBAYAR_CODX-DJJWHJGQB'  # Add your credentials here
...
def validate_inputs(team_code, uids, emote_id):
    """Validate all inputs according to requirements"""
    errors = []
    
    # Validate team code (5-9 digits, numeric only)
    if not team_code:
        errors.append("Team code is required")
    elif not team_code.isdigit():
        errors.append("Team code must contain only numbers")
    elif len(team_code) < 5:
        errors.append("Team code must be at least 5 digits")
    elif len(team_code) > 9:
        errors.append("Team code must be maximum 9 digits")
    
    # Validate UIDs (7-12 digits, numeric only)
    if not uids:
        errors.append("At least one UID is required")
    else:
        for i, uid in enumerate(uids, 1):
            if not str(uid).isdigit():
                errors.append(f"UID{i} must contain only numbers")
            elif not (7 <= len(str(uid)) <= 12):
                errors.append(f"UID{i} must be between 7-12 digits")
    
    # Validate emote ID (numeric only, any positive number)
    if not str(emote_id).isdigit():
        errors.append("Emote ID must contain only numbers")
    elif int(emote_id) < 1:
        errors.append("Emote ID must be a positive number")
...
def join_and_emote():
    ...
    """
    try:
        team_code = request.args.get('tc', '').strip()
        emote_id_str = request.args.get('emote_id', '909000001')
        ...
        # Convert emote_id to int
        try:
            emote_id = int(emote_id_str)
        except ValueError:
            return jsonify({
                "status": "error",
                "message": "Invalid emote ID format"
            }), 400
        
        # Validate all inputs
        validation_errors = validate_inputs(team_code, uids, emote_id)
